...
class pesticideReports extends JFrame implements WindowListener,ActionListener{
	...
	public pesticideReports(Register r){
		super("Farm Management System/PesticideReports");
		this.register=r;
 ...
 l0=new Label("Pesticide Reports");
l0.setFont(new Font("Pesticide Reports",Font.PLAIN,20));		 
	...
	public void actionPerformed(ActionEvent e){
		...
		int l=0;
		if(s.equals("Home")){
			register.pesticidereports.setVisible(false);
		...
		if(s.equals("Show All")){
			...
			ResultSet rs=da.getData("SELECT * FROM pesticide");
			messageBox.setText(null);
			String st="<html>";
			while(rs.next()){
				st=st+"  PesticideId:  "+rs.getString("Pid")+"  PesticideName:  "+rs.getString("Pname");
				st=st+"  Description:   "+rs.getString("Description")+"  Quantity: "+rs.getString("Quantity")+"<br/>";
			}
			st+="</html>";
		...
		if(s.equals("Delete")){
			...
			while(rs.next()){
				...
				if(d.equals(tf8.getText())){					
					String sql="DELETE FROM  pesticide WHERE Pid = '"+tf8.getText()+"'";
				da.updateDB(sql);
