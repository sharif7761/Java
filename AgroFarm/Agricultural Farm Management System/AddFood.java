...
class AddFood extends Frame implements WindowListener,ActionListener{
	...
	public AddFood(Register r){
		super("Farm Management System/Addfood");
		this.register=r;
 ...
 l0=new Label("Add Food Details");
l0.setFont(new Font("Add Food Details",Font.PLAIN,20));		 
		  l0.setBounds(20,180, 500,45); 
		  		
		 l1=new Label("Food Id");  
		  l1.setFont(new Font("Food Id",Font.PLAIN,20));
		  l1.setBounds(30,230, 150,30);  
		tf=new TextField();  
        tf.setBounds(240,230, 150,30);
		
		l2=new Label("Food Type");  
		  l2.setFont(new Font("Food Type",Font.PLAIN,20));
		...
		add(tf);add(l1);add(tf1);add(l8);add(tf2);add(l2);add(tf3);add(l3);
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		if(st.equals("Submit")){
			String sql="insert into food (Fid,Fname,Type,Quantity) values('"+tf.getText()+"','"+tf2.getText()+"','"+tf1.getText()+"','"+tf3.getText()+"')";
				da.updateDB(sql);
			...
			if(st.equals("Home")){
			register.addfood.setVisible(false);
