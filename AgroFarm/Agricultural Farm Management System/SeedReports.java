...
class SeedReports extends JFrame implements WindowListener,ActionListener{
 ...
 l0=new Label("Seed Reports");
l0.setFont(new Font("Seed Reports",Font.PLAIN,20));		 
	...
	public void actionPerformed(ActionEvent e){
		...
		int l=0;
		if(s.equals("Home")){
			register.seedreports.setVisible(false);
		...
		if(s.equals("Show All")){
			...
			ResultSet rs=da.getData("SELECT * FROM seed");
			messageBox.setText(null);
			String st="<html>";
			while(rs.next()){
				st=st+"  SeedId: "+rs.getString("Sid")+"  SeedName:  "+rs.getString("Sname");
				st=st+"  SeedType:  "+rs.getString("Type")+"  Quantity: "+rs.getString("Quantity")+"<br/>";
			}
			st+="</html>";
		...
		if(s.equals("Delete")){
			...
			while(rs.next()){
				...
				if(d.equals(tf8.getText())){					
					String sql="DELETE FROM  seed WHERE Sid = '"+tf8.getText()+"'";
				da.updateDB(sql);
