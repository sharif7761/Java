...
import java.sql.*;
class EmployeeReports extends JFrame implements WindowListener,ActionListener{
	...
	public EmployeeReports(Register r){
		super("Farm Management System/EmployeeReports");
		this.register=r;
 ...
 l0=new Label("Employee Reports");
l0.setFont(new Font("Employee Reports",Font.PLAIN,20));		 
	...
	public void actionPerformed(ActionEvent e){
		...
		int l=0;
		if(s.equals("Home")){
			register.employeereports.setVisible(false);
		...
		if(s.equals("Show All")){
			...
			ResultSet rs=da.getData("SELECT * FROM employee");
			messageBox.setText(null);
			String st="<html>";
			while(rs.next()){
				st=st+"  EmployeeId:  "+rs.getString("Eid")+"  FullName:  "+rs.getString("FullName");
				st=st+"  Address:  "+rs.getString("Address")+"  Phone:  "+rs.getString("Phone");
				st=st+"  Position:   "+rs.getString("Position")+"  Salary: "+rs.getString("Salary")+"<br/>";
			}
			st+="</html>";
		...
		if(s.equals("Delete")){
			...
			while(rs.next()){
				...
				if(d.equals(tf8.getText())){					
					String sql="DELETE FROM  employee WHERE Eid = '"+tf8.getText()+"'";
				da.updateDB(sql);
