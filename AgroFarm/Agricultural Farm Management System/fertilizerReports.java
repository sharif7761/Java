...
class fertilizerReports extends JFrame implements WindowListener,ActionListener{
	...
	public fertilizerReports(Register r){
		super("Farm Management System/fertilizerReports");
		this.register=r;
 ...
 l0=new Label("Fertilizer Reports");
l0.setFont(new Font("Fertilizer Reports",Font.PLAIN,20));		 
	...
	public void actionPerformed(ActionEvent e){
		...
		if(s.equals("Show All")){
			...
			ResultSet rs=da.getData("SELECT * FROM fertilizer");
			messageBox.setText(null);
			String st="<html>";
			while(rs.next()){
				st=st+"  FertilizerId: "+rs.getString("Frid")+"  Name:  "+rs.getString("Frname");
				st=st+"  Rate:  "+rs.getString("Rate")+"  Quantity: "+rs.getString("Quantity")+"<br/>";
			}
			st+="</html>";
		...
		if(s.equals("Delete")){
			...
			ResultSet rs=da.getData("SELECT * FROM fertilizer ");
			while(rs.next()){
				String d=rs.getString("Frid");
				if(d.equals(tf8.getText())){					
					String sql="DELETE FROM  fertilizer WHERE Frid = '"+tf8.getText()+"'";
				da.updateDB(sql);
