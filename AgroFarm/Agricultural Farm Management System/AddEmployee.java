...
class AddEmployee extends Frame implements WindowListener,ActionListener{
	...
	public AddEmployee(Register r){
		super("Farm Management System/Addemployee");
		this.register=r;
 ...
 l0=new Label("New Employee Information");
l0.setFont(new Font("New Employee Information",Font.PLAIN,20));		 
        ...
        submitButton.setBounds(240,510,60,30);  
		
		 resetButton1=new Button("Reset");  
        resetButton1.setBounds(320,510,60,30);  
		
		
       
        add(l);add(l0);add(b);add(b1);add(b2);add(b3);add(b4);add(b5);add(submitButton);add(resetButton1);
		add(tf);add(l1);add(l0);add(tf1);add(l8);add(tf2);add(l2);add(tf3);add(l3);add(tf4);add(l4);add(tf5);add(l5);
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		if(st.equals("Submit")){
			String sql="insert into employee (Eid,FullName,Address,Position,Phone,Salary) values('"+tf1.getText()+"','"+tf.getText()+"','"+tf2.getText()+"','"+tf4.getText()+"','"+tf3.getText()+"','"+tf5.getText()+"')";
				da.updateDB(sql);
		...
		if(st.equals("Home")){
			register.addemployee.setVisible(false);
