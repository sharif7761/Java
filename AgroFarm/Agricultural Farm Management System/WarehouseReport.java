...
class WarehouseReport extends JFrame implements WindowListener,ActionListener{
...
public JLabel messageBox;	   
	public WarehouseReport(Register r){
		super("Farm Management System/Report/WarehouseReport");
		this.register=r;
...
delete=new Button("Delete");  
 ...
 l0=new Label("Warehouse Reports");
l0.setFont(new Font("Warehouse Reports",Font.PLAIN,20));		 
	...
	public void actionPerformed(ActionEvent e){
		...
		if(s.equals("Home")){
			register.warehousereports.setVisible(false);
		...
		if(s.equals("Show All")){
			...
			ResultSet rs=da.getData("SELECT * FROM warehouse");
			messageBox.setText(null);
			String st="<html>";
			while(rs.next()){
				st=st+"  WarehouseId: "+rs.getString("Wid")+"    WarehouseName:  "+rs.getString("Wname");
				st=st+"    FoodName:  "+rs.getString("FoodName")+"    Quantity: "+rs.getString("Quantity")+"<br/>";
			}
			st+="</html>";
		...
		if(s.equals("Delete")){
			...
			ResultSet rs=da.getData("SELECT * FROM warehouse ");
			while(rs.next()){
				String d=rs.getString("Wid");
				if(d.equals(tf8.getText())){					
					String sql="DELETE FROM  warehouse WHERE Wid = '"+tf8.getText()+"'";
				da.updateDB(sql);
